...
From the confusion matrix, we found that the logistic model has a high in-sample accuracy of 0.9253, with sensitivity of 0.9121 and specificity of 0.9398. It can be said that the logisic model did quite well at least for the given data. The high accuracy did not sacrifice sensitivity nor specificity (this may thank to the quite even distribution of `real` and `fake` cases).
...
We then get the cross-validated accuracy of the logistic model: 0.8157983, which means the possible accuracy we would like to get if we use this model to predict the authenticity of new news data. This is aroung 0.1 lower than the in-sample error, but is still high, so we consider `logistic_model` a good choice at this stage.
...
It's surprising to find the best tuned k is 1 and the in-sample accuracy is 1, which means the highest accuracy is achieved by only looking at one nearest neighbor and this did avoid any in-sample error. 
...
Just as what we condidered above, the CV accuracy is 0.22 lower than the in-sample error, which means the in-sample modeling is a bit overfitting. But generally speaking, the CV accuracy of 0.7810084 is fairly high.
...
We can find that deleting predictors did not decrease the accuracies in logistic and knn models. Therefore, we would prefer to use `logistic_model_3`, the most computationally efficient algorithm with 6 predictors: `TextNumOfQuotation`, `TitleLength`, `disgust`, `surprise`, `positive`, `TextLength`.
...
`forest_model`    |      0.7816            |    0.8046         |     all
...
`forest_model_2`  |      0.7198            |    0.7198         |     without source
...
Therefore, we will visualize the relationship between the four most important variables (`TextNumOfQuotation`, `TitleLength`, `disgust`, and `surprise`) and the outcome variable y(type in this case).

To better evaluate the ranking information and understand the relationship between features, we draw a heatmap showing the correlation between the variables:
...
4. Logistic model is a fairly good classification in all senarios. 
